...
* `TabControl`（containers）标签条按 Navicat 实测重做溢出与配色：
  ① 新增 `pinnedCount`——条首前 N 个标签排在滚动视口**之外**，永不随滚动位移（宿主用它钉住主入口页）。固定标签照常参与选中与键盘导航，`_ensureVisible` 对它们直接跳过；判定「要不要出箭头」时先把它们的宽度从可用宽度里扣掉，否则固定块会被算进可滚动内容、标签稍多就误判溢出。
  ② 修**标签多到溢出时两个箭头一个都不出现**（整条标签再也滚不到）：两端可用性原先只挂在滚动监听上，而监听要等用户已经滚过才触发。现在溢出判定与两端可用性统一由测宽回调 `_syncScrollMetrics` 结算（滚动监听只刷新箭头），并且只要溢出就**恒定画出**两个箭头、滚不动的一侧置灰——按「能不能滚」决定显隐会让箭头在触底时突然消失，整条标签左右抽搐 24px。左箭头紧跟在固定块右侧，不摆到整条最左端。
  ③ 滚轮压在标签条上即横向滚动（`Listener.onPointerSignal`，不必去够那对小箭头）；键盘 / 右键菜单切到被裁掉的标签时自动 `_ensureVisible` 滚进可视区（延到下一帧，重排尚未完成）。
  ④ 新增 `unselectedTabColor`：未选中标签底色可显式钉死。缺省仍是「条底向 `surfaceColor` 提亮一档」，但宿主 `surfaceColor == background` 且要求白色标签条时，提亮结果会落在 `#FCFCFC~#FDFDFD` 这类近白上、整条发脏——纯白底必须直接传纯白。
  ⑤ 底线不再叠成 2px：带正文的标签条由 `TabControl` 画一条横贯全宽的发丝线，未选中标签**不再**自画底线（`_TabHeader.stripBottomLine`）；纯标签条没有那条全宽线，每个格子才自己补，否则一排标签悬在白底下沿全无交代。标签条底线与页面面板轮廓改用 `buttonBorderColor`（比通用细线深一档）——面色是白的时候用 `borderColor` 会糊成一片，与 Button「面色变白后补回轮廓感」同一条规则。回归测试见 `test/tab_control_pinned_test.dart`（固定标签不随滚动位移 / 不设 `pinnedCount` 时逐字节等价旧版 / 固定宽度不计入可滚动内容故不误判溢出 / 箭头格有顶边与底边）与 `test/tab_control_chrome_test.dart`（纯标签条只给未选中标签画底线、带正文的标签条不重复画）。

* 日期时间选择器按 Navicat 重排，并把日历升级为**三级快选**：
  ① `MonthCalendar`（dialogs）顶栏标题改为可点——日视图点标题进月份快选（4×3），月视图点标题进年份快选（12 年一档），再点回日视图；左右箭头按**当前视图**步进一档（月 / 年 / 12 年）。日格固定铺满 6 周，月外日子按参考图画成灰显且**可点**（选中该日并翻到所属月），底部补一行「今天: yyyy/m/d」图例。`weekdayLabels` 参数取消，统一改走 `labels`。
  ② 新增 `TimeField`（form）：单行 `HH:MM:SS` + 右缘竖排上下按钮，替掉弹窗里「时 / 分 / 秒」三行 `NumericUpDown`。三段各自可点为当前段（强调色底），上下按钮 / 滚轮 / ↑↓ 只作用在当前段并按 24 / 60 进制回环；← → 切段，Home / End 跳两端，数字键累加（装不下第二位时自动前进到下一段，拼不出合法值时以新数字重启该段），Backspace 退回上一段并清零。刻意不用 `TextField`：IME 与选区会搅乱「哪一段是当前段」，而取值永远是定宽三段。
  ③ `DateTimePickerLabels` **破坏性调整**：去掉 `time` / `hour` / `minute` / `second`（三行微调没了），新增 `today` / `monthNames` / `monthTitle` / `yearTitle` / `yearRangeTitle`。三个标题是**模板**而非格式化函数（`{year}` / `{month}` / `{monthName}` / `{from}` / `{to}` 在渲染时由 `fill()` 替换），`const` 构造因此仍然成立，中文 `'{year}年{month}月'` 与英文 `'{monthName} {year}'` 的语序差异各说各话。宿主侧注意：gen-l10n 的 ARB 不允许正文出现裸 `{}`（会当占位符解析），故 daro 的 ARB 用 `[year]` 方括号书写、由宿主换成花括号。
  ④ 三个弹窗（date / time / dateTime）改用共用的 `_PickerShell`（264 / 200 定宽、`controlColor` 底、`compactSpacing * 2` 内边距）与居中的「确定 + 取消」动作行，与居中的「确定 + 取消」动作行，与参考图留白一致。回归测试见 `test/month_calendar_test.dart`（三级下钻与回退 / 箭头按视图步进 / 邻月灰显日可点并翻月 / 区间把整月整年一起禁用）、`test/time_field_test.dart`（段命中 / 回环 / 累加前进 / Backspace / 滚轮 / 禁用态），以及 `test/inline_editor_date_picker_test.dart` 里新增的整弹窗用例（打开时显示**值所在的月**、标题下钻选月、TimeField 改分，确认后的合并值 `2022-03-25 20:14:06` 三项各自归位、未动的秒沿用原值）。演示页 `calendar.quickPickHint` 补了一句「点标题可快速选月 / 年」。

* `DataGridView`（lists）新增**可选横向虚拟化** `horizontalScrollController` + `horizontalViewportWidth`：宽表把整个网格放进外层横向 `SingleChildScrollView` 时，每一行的 `Row` 原先会把**所有列**都建成真实 widget（哪怕横向滚出视口），于是「勾选列 / 选择 / 拖拽改列宽」等任何触发网格重建的操作，成本 ∝ 可见行 × 全部列（daro 表数据页 21 列 × 约 36 行 ≈ 750 格 / 帧，勾选时肉眼可见掉帧）。同时传入横向滚动控制器与视口宽后，数据行只渲染与视口相交的列，用左右占位条撑出等宽（横向滚动范围、`_colAtX` 命中测试、选中坐标均不受影响），把每帧重建量降到 ∝ 可见行 × 视口内列。**默认关闭**——不传这两个参数时逐字节等价于旧版，表头始终整行渲染（列宽调整 / 排序手柄需常驻，且仅 1 行成本可忽略）。回归测试见 `test/data_grid_hvirtual_test.dart`（视口外列不建 widget / 横向滚动换窗 / 换窗后点击命中正确列 / 行号列随内容滚出 / 无视口宽回退整行）。

* `Input`（common）修**字段内按钮点不动**：`trailing`（行内编辑器的日历/时钟按钮）与 `obscureToggle`（密码眼睛）此前直接挂在 `Row` 里，落在 `TextField` 自带的 `TextFieldTapRegion` 之外——按下那一下被 `EditableText` 判成「点到框外」，默认行为是直接失焦。宿主只要把「失焦」当提交（`InlineEditor` 就是），编辑器会在抬起之前被拆掉，按钮的 `onTap` 永远收不到，表现为「点日历没反应」。表格里按下必然伴随宿主选中重绘（多出一帧），所以只在单元格内稳定复现、单独摆一个编辑器反而正常。现在两者都包进 `TextFieldTapRegion`，与输入框同属一个点击区。回归测试见 `test/inline_editor_date_picker_test.dart`（按下 → 渲染一帧 → 抬起的真实节奏下弹窗照常打开、且不触发失焦提交；去掉包装即红）。

* 新增 `buildTextBoxContextMenu(context, editable, {tokens})`（menus）与 `ContextMenuPanel`：`Input` 的右键菜单不再用 Flutter 自带的 Material 选择工具条（圆角浮层 + 图标按钮，和全库的 Win10 菜单不是一套），改走本库面板渲染。剪切 / 复制 / 粘贴 / 全选**恒定列出**，当前做不了的置灰而不是隐藏（可用性仍取自 `EditableTextState.contextMenuButtonItems`，它才知道只读框不能粘贴、无选区不能复制），快捷键标在右缘，菜单左上角落在鼠标按下处并在屏幕边缘折回；标签取 `MaterialLocalizations`（`cutButtonLabel` 等，因此跟随宿主语言），动作与收起都交回 `EditableTextState`（`onPressed` / `hideToolbar`）。粘贴项可用性依赖异步回来的剪贴板状态，故面板包一层 `ValueListenableBuilder(clipboardStatus)`，状态到达后自动从置灰变可用。`ContextMenuPanel` 是从 `showContextMenu` 抽出的面板本体（边框 + 条目列，不含定位与点击外部关闭的屏障），两处共用同一套绘制。回归测试见 `test/text_box_context_menu_test.dart`（弹的是本库面板且四项全在 / 点「全选」真的全选并收起；去掉 `contextMenuBuilder` 即红）。

* `ScrollBar`（scroll）新增 `notificationPredicate`（透传给 Material Scrollbar）：修**纵向条挂在横向滚动区外面时整条不显示**。Material 的 Scrollbar 默认只接收 `depth == 0` 的滚动通知，宽表网格的标准摆法（纵向条包住横向 `SingleChildScrollView`，内层再放纵向 ListView）中，内层通知冒泡经过横向 Scrollable 会被记为 depth 1、全被默认过滤器拒收，纵向条因永远拿不到尺寸而不绘制。宿主传 `notificationPredicate: (n) => n.metrics.axis == Axis.vertical` 按轴过滤即可。回归测试见 `test/scroll_bar_test.dart`（嵌套结构下断言右缘画出轨道/滑块矩形；去掉 predicate 即红）。
...
* `DesktopTokens` 新增**按钮专用令牌组**：`buttonHeight`（默认 24）/ `buttonMinWidth`（默认 73）/ `buttonFaceColor` / `buttonBorderColor` / `buttonHoverFaceColor` / `buttonHoverBorderColor` / `buttonPressedFaceColor` / `buttonPressedBorderColor`（winForm 与 shadcn 两版预设均已给值）。**刻意与 `controlHeight`(28) 分离**：桌面工具包的按钮总比单行编辑框矮一档（Navicat 实测按钮 73×24、输入框高 19.5），改按钮不应连带改到输入框 / 下拉 / 复选。`Button` 的 `solid` 变体接入：常态面取 `buttonFaceColor`、边取 `buttonBorderColor`（比通用控件边线略深一档，补回面色变白后丢掉的轮廓感）；hover / pressed 走专用热态令牌，规则是「**染强调色**」而不是「把中性面色压暗」—— 后者只能叠出灰，越悬停越脏，参考图实测悬浮面 #E0EEF9 + 边 #0078D4、按下再深一档。**下边框恒比上 / 左 / 右暗 ~10.5%**（桌面按钮 VCL/Aero 的 3D 下缘；参考图常态 #D0D0D0→#BABABA、悬浮 #0078D4→#006BBE 同一比例），由 `Color.alphaBlend(0x1B000000, 边色)` 组件内现算，不另加令牌。只有 `child == null` 的**纯文字**按钮吃 `buttonMinWidth`（桌面对话框把「确定 / 取消 / 上一步」摆在等宽按钮列上，不会贴着字收紧）；带 `child` 的工具条 / 图标按钮一律按内容自适应，否则窄工具条会被 73 撑破。`ghost` / `primary` 变体与禁用态外观不变。

* 新增 `DesktopTokens.withHairlineBorders(BuildContext context)`：把 `borderWidth` 按 `1 / devicePixelRatio` 收窄（下限 0.25），使边框恒定占 **1 设备像素**。参照物是原生窗口（Navicat / VCL），其边框恒为 1 **物理**像素、与显示缩放无关，而令牌里的 `borderWidth = 1.0` 是**逻辑**像素 —— 200% 缩放下会画成 2 物理像素，比原生窗口粗一倍（实测 daro 截图描边占 2 物理像素、Navicat 占 1）。单 DPI(100%) 下返回 1.0，行为与改前完全一致，只有高 DPI 屏才变。拿不到 `MediaQuery` 时原样返回。

* 组件里**不再写死线宽**：`CheckBox` 的方框、`MarqueeSelector` 的选框、`StepBar` 的连接线（`Container(height: 1)`）、`ThemeDesigner` 导出页输入框的描边，原先分别写死 `width: 1` / `height: 1`，改为取 `t.borderWidth`。这些在 200% 屏幕上会明显比周围边框粗，且不收窄的话「1 设备像素边框」策略对它们无效。

* `DialogBox` 底部动作条改为**浅灰带**：底色取 `secondaryColor`、上方 1px `borderColor` 分隔线、内边距 10/9（Navicat 实测），整条铺满对话框宽度（内边距由 `Container` 提供，浅灰底一直顶到对话框边框）。按钮面色朝纸白、压在浅灰条上，层次才出得来 —— 此前按钮与正文同底色，只靠一条空隙分隔，整块读起来是平的。

* 新增 `ThresholdDraggable`（lists）：**只有真的拖了才起手**的 `Draggable`。框架 `Draggable` 有两条「没拖就先起手」的路径，桌面端单击 / 双击都会闪拖拽特效：① **竞技场默认接受**——本识别器是竞技场唯一成员时（桌面端鼠标点一个拖拽源通常就是这种情况，`Scrollable` 的拖拽识别器默认不接受鼠标），`GestureArenaManager.close()` 直接把这一手判给它，`onDragStarted` 在 pointer down 的微任务里就触发了，指针一寸没动而浮层预览、`childWhenDragging` 与宿主据此点亮的放置区已全部出现，松手再收回；② **鼠标起手容差只有 1px**——框架对鼠标硬编码 `kPrecisePointerHitSlop`（只有触屏 / 触控板才读 `MediaQuery.gestureSettings.touchSlop`），单击 / 双击时几像素手抖即可判定成拖动。典型症状即「双击打开一行却闪出拖拽浮层」。本组件把阈值提高到 `startSlop`（默认 `kDragStartSlop` = 4px，与 Windows 的 `SM_CXDRAG` 及本库 `MarqueeSelector.threshold` 同量级），并把「竞技场判给本识别器」与「起手」分开：输了手势竞争不算，赢了但位移没到阈值也只挂起起手回调，位移够了才真正起手、不够就作废（不闪浮层、不触发 `onDragStarted`）。按下即拖的手感不变（不引入 `LongPressDraggable` 那种时间延迟），位移累计值在真正起手时一次性补报给浮层，拖起来不跳。构造函数逐字段对齐 `Draggable`，可直接替换。配套导出 `ThresholdMultiDragGestureRecognizer`（`affinity` 一个字段覆盖任意 / 水平 / 垂直三个方向，宿主用自己的 `RawGestureDetector` 时可直接用）。回归测试见 `test/threshold_draggable_test.dart`（唯一成员时单纯按下不起手 / 鼠标 3px 手抖不起手 / 位移跨过阈值才起手并落到 `DragTarget` / 有竞争识别器时同样成立）。
